...
class CategoryController @Inject()(val controllerComponents: ControllerComponents) extends BaseController {
  ...
  def getById(id: Int) = Action {
    categories.find(_.id == id) match {
      ...
      case None => NotFound(Json.obj("error" -> "getById - Category not found"))
    }
  }

  def add = Action(parse.json) { request =>
    request.body.validate[Category].fold(
      _ => BadRequest(Json.obj("error" -> "add - Invalid category")),
      category => {
        categories += category
  ...
  def update(id: Int) = Action(parse.json) { request =>
    request.body.validate[Category].fold(
      _ => BadRequest(Json.obj("error" -> "update - Invalid category")),
      updated => {
        categories.indexWhere(_.id == id) match {
          case -1 => NotFound(Json.obj("error" -> "update - Category not found"))
          case i =>
  ...
  def delete(id: Int) = Action {
    categories.indexWhere(_.id == id) match {
      case -1 => NotFound(Json.obj("error" -> "delete - Category not found"))
      case i =>
